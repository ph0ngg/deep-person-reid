...
class OSNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.conv51 = Conv1x1(channels[3], channels[3])
        self.conv52= Conv1x1(channels[3], channels[3])
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.linear = nn.Linear(512, 512)
        self.global_avgpool = nn.AdaptiveAvgPool2d(1)
        self.tanh = nn.Tanh()
        self.conv1x1 = Conv1x1(512, 512, 1, 1)
    ...
    def FENN(self, x):
        #print(pose(x))
        crop_head_img, crop_larm_img, crop_rarm_img, crop_body_img, crop_lleg_img, crop_rleg_img = pose(x)  
        l_arm = self.fen(crop_larm_img)
        r_arm = self.fen(crop_rarm_img)
        l_leg = self.fen(crop_lleg_img)
        r_leg = self.fen(crop_rleg_img)
        body = self.fen_body(crop_body_img)
        y1 = torch.cat((l_arm,l_leg), axis = 2)
        y2 = torch.cat((r_arm,r_leg), axis = 2)
        black_area = torch.zeros((8, 3, 128, 128)).cuda(device= 'cuda:0')
    ...
    def featuremaps(self, x):
        ...
        return global_feature, part_feature, fused_feature

    def forward(self, x, return_featuremaps=False):
        global_feature, part_feature, fused_feature = self.featuremaps(x)
        global_feature = self.global_avgpool(global_feature)
        global_feature = global_feature.view(global_feature.size(0), -1)
        part_feature = self.global_avgpool(part_feature)
        part_feature = part_feature.view(part_feature.size(0), -1)
        fused_feature = fused_feature.view(fused_feature.size(0), -1)
        fused_feature = self.classifier1(fused_feature)
        global_feature = self.classifier2(global_feature)
        part_feature = self.classifier2(part_feature)
